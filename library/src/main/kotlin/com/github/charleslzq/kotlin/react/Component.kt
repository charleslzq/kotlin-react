...
open class Component<V, S>(val view: V, val store: S) where V : View {
    ...
    inline fun <reified T, SV, SS> getChildren(): List<T> where T : Component<SV, SS> = children.list.filter { it is T }.map { it as T }

    inline fun <P> render(property: KProperty1<S, P>,
    ...
    inline fun <P> render(property: KProperty0<P>,
                          scheduler: Scheduler = AndroidSchedulers.mainThread(),
                          crossinline require: () -> Boolean = { true },
                          crossinline handler: (P) -> Unit)
    ...
    inline fun renderByAll(vararg properties: KProperty1<S, *>,
                           scheduler: Scheduler = AndroidSchedulers.mainThread(),
                           crossinline require: () -> Boolean = { true },
                           crossinline handler: () -> Unit)
            = properties.forEach { render(it, scheduler, require) { handler() } }

    inline fun renderByAll(vararg properties: KProperty0<*>,
                           scheduler: Scheduler = AndroidSchedulers.mainThread(),
                           crossinline require: () -> Boolean = { true },
                           crossinline handler: () -> Unit)
            = properties.forEach { render(it, scheduler, require) { handler() } }
    ...
    class Children<V, S>(private val view: V, private val store: S) where V : View {
        @PublishedApi internal val list = mutableListOf<Component<*, *>>()
        @PublishedApi internal val binders = mutableListOf<() -> List<Component<*, *>>>()
        ...
        fun <T, SV, SS> children(findViews: (V) -> List<SV>, generator: IndexedContext<SV, S>.() -> T) where SV : View, T : Component<SV, SS>
                = binders.add { findViews(view).mapIndexed { index, sv -> with(IndexedContext(sv, store, index), generator) } }
